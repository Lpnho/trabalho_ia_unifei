...
template <std::size_t N,
          std::size_t POPULATION_SIZE, std::size_t GENERATION_N = 200>
class GeneticAlgorithm
{
    ...
    double _fitness[BUFFERS_COUNT][POPULATION_SIZE];

public:
    GeneticAlgorithm &initPopulation(std::size_t currentPopulation)
    {
        ...
        for (std::size_t iPop = 0; iPop < POPULATION_SIZE; ++iPop)
        {
            for (std::size_t iN = 0; iN < N; ++iN)
            {
                _population[currentPopulation][iPop][iN] = random(_gerador);
            }
            _fitness[currentPopulation][iPop] = applyFitness(_population[currentPopulation], iPop);
        }
        return *this;
    }
    ...
    inline std::size_t tournamentSelection(std::size_t currentPopulation)
    {
        ...
        double maiorFit = _fitness[currentPopulation][elements[0]];
        ...
        for (std::size_t index = 1; index < NT; ++index)
        {
            ...
            fit = _fitness[currentPopulation][elements[index]];
            if (fit > maiorFit)
            {
                maior = index;
                maiorFit = fit;
    ...
    inline void run(std::string outFile, std::size_t precision = 8)
    {
        ...
        thread_local std::mt19937 gerador{randomDevice()};
        thread_local std::uniform_real_distribution<double> randomDouble(0.0, 1.0);
        thread_local std::uniform_int_distribution<std::size_t> random(0, POPULATION_SIZE - 1);

        for (std::size_t geracoes = 0; geracoes < GENERATION_N; ++geracoes)
        {
            // currentBestFitness = applyFitness(_population[currentPopulation], 0);
            double currentBestFitness = _fitness[currentPopulation][0];
            std::size_t currentBestIndex = 0;
#pragma omp parallel
            {

                double dads[CROSSOVER_U][N]{};
                double localBestFitness = _fitness[currentPopulation][0];
                std::size_t localBestIndex = 0;

#pragma omp for
                for (std::size_t individuo = 1; individuo < POPULATION_SIZE; ++individuo)
                {
                    bool crossOverOrMutation = false;
                    double value = _fitness[currentPopulation][individuo];

                    if (value < localBestFitness)
                    {
                        localBestFitness = value;
                        localBestIndex = individuo;
                    }
                    ...
                    if (randomDouble(gerador) < CROSSOVER_RATE)
                    {
                        ...
                        {
                            std::size_t randDad = tournamentSelection<TOURNAMENT_N>(currentPopulation);
                            for (std::size_t iN = 0; iN < N; ++iN)
                            {
                    ...
                    _fitness[bufferPopulation][individuo] = applyFitness(_population[bufferPopulation], individuo);
                }

#pragma omp critical
                {
                    if (currentBestFitness > localBestFitness)
                    {
                        currentBestIndex = localBestIndex;
                        currentBestFitness = localBestFitness;
                    }
                }
            }
            ...
            _fitness[bufferPopulation][0] = applyFitness(_population[bufferPopulation], 0);
...
int main()
{
    ...
    GeneticAlgorithm<10, 10000, 1200>().run("10-10000-1200.txt", 16);
